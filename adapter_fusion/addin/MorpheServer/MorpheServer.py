...
def _setup_import_paths() -> tuple[bool, str]:
    """
    Set up import paths to find adapter_fusion and core.
    ...
    expected_canonical_dir = os.path.join(repo_dir, "core")

    if not os.path.exists(expected_adapter_init):
        return False, (
            f"Could not find adapter_fusion package.\n"
    ...
    if not os.path.isdir(expected_canonical_dir):
        return False, (
            f"Could not find core package.\n"
            f"Expected directory at: {expected_canonical_dir}\n\n"
            f"Please ensure core is in the repository root."
        )
...
def run(context: dict) -> None:
    ...
    try:
        ...
        from adapter_fusion.server import is_server_running, start_server
...
def stop(context: dict) -> None:
    ...
    try:
        if _server_started and _paths_ok:
            from adapter_fusion.server import is_server_running, stop_server
