...
Example usage (on Windows with SolidWorks installed):
    ...
    from adapter_solidworks import SolidWorksAdapter
    from core import SketchDocument, Line, Point2D
...
RPC Server (run as external process with SolidWorks running):
    from adapter_solidworks import start_server
    start_server()  # Starts on localhost:9878

RPC Client (connect to server):
    from adapter_solidworks import SolidWorksClient
